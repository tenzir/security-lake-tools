...
def get_current_account_id(session: boto3.Session) -> Optional[str]:
...
def create_custom_source(
    ...
    print()
    print(f"ℹ Note: Security Lake will create the provider role: AmazonSecurityLake-Provider-{source_name}-{region}")
    print()
    ...
    try:
        ...
        if error_code == 'ResourceAlreadyExistsException':
            ...
            print(f"  • Or specify an existing role with --glue-role-arn")
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a custom log source in Amazon Security Lake for a specific OCSF event class",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s 1001 --region us-east-1 --account-id 123456789012 --external-id my-external-id --glue-role-arn arn:aws:iam::123456789012:role/MyGlueRole
  %(prog)s 4003 --external-id my-external-id  # Uses auto-detected account ID and default Glue role
  %(prog)s --list  # List all available OCSF class UIDs
    ...
    parser.add_argument(
        "class_uid",
        nargs="?",
        help="OCSF class UID (e.g., 1001 for File System Activity)"
    )
    ...
    parser.add_argument(
        "--account-id",
        help="AWS account ID (default: auto-detected using boto3)"
    )
    ...
    parser.add_argument(
        "--glue-role-arn",
        help="ARN of the Glue service role (default: arn:aws:iam::{account-id}:role/AWSGlueServiceRole-SecurityLake)"
    )
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="List all available OCSF class UIDs and exit"
    )
    
    parser.add_argument(
        "--skip-role-check",
        action="store_true",
        help="Skip verification that the Glue role exists"
    )
    ...
    if args.list:
        ...
        categories = [
            ("System Activity", ["1001", "1002", "1003", "1004", "1005", "1006", "1007", "1008", "1009"]),
            ("Findings", ["2001", "2002", "2003", "2004", "2005", "2006", "2007"]),
            ("Identity & Access Management", ["3001", "3002", "3003", "3004", "3005", "3006"]),
            ("Network Activity", ["4001", "4002", "4003", "4004", "4005", "4006", "4007", "4008", "4009", "4013", "4014"]),
    ...
    if not args.skip_role_check:
        ...
        if not verify_glue_role(session, glue_role_arn):
            ...
            if args.no_create_role:
                ...
                print("  3. A policy for S3 access to Security Lake buckets (aws-security-data-lake-*)")
                print("\nAlternatively:")
                ...
                print(f"ℹ Glue role does not exist, creating it automatically...")
